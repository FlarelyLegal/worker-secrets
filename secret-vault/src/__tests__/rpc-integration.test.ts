...
import { FLAG_MAINTENANCE, FLAG_READ_ONLY, SCOPE_ALL, SCOPE_READ } from "../constants.js";
...
async function buildTestCtx(overrides?: Partial<AuthUser>): Promise<ServiceContext> {
  const auth: AuthUser = {
    method: "rpc",
    identity: "test-rpc",
    name: "test-rpc",
...
describe("Maintenance mode enforcement", () => {
  it("blocks secret reads when maintenance flag is set", async () => {
    ...
    await env.FLAGS.put(FLAG_MAINTENANCE, JSON.stringify({ value: true, type: "boolean" }));
  ...
  it("allows operations when maintenance flag is false", async () => {
    await env.FLAGS.put(FLAG_MAINTENANCE, JSON.stringify({ value: false, type: "boolean" }));
...
describe("Read-only mode enforcement", () => {
  it("read-only flag is picked up by loadAllFlags", async () => {
    await env.FLAGS.put(FLAG_READ_ONLY, JSON.stringify({ value: true, type: "boolean" }));
    const flagCache = await loadAllFlags(env.FLAGS);
...
describe("RBAC enforcement via restricted role", () => {
  it("reader role can read secrets but not write", async () => {
    ...
    await expect(secretsService.deleteSecret(readerCtx, "rbac-read-test")).rejects.toThrow(
      AccessDeniedError,
    );
  ...
  it("tag-restricted role can only access matching tags", async () => {
    ...
    await expect(secretsService.getSecret(billingCtx, "other-tagged-secret")).rejects.toThrow(
      AccessDeniedError,
    );
...
describe("Full CRUD lifecycle", () => {
  it("complete secret lifecycle through RPC service context", async () => {
    ...
    const { versions } = await versionsService.listVersions(ctx, "lifecycle-key");
    expect(versions.length).toBeGreaterThanOrEqual(1);

    // Get version — should contain original value
    const ver = await versionsService.getVersion(ctx, "lifecycle-key", versions[0].id);
    expect(ver.value).toBe("v1-value");

    // Restore — bring back v1
    const restored = await versionsService.restoreVersion(ctx, "lifecycle-key", versions[0].id);
    ...
    // Verify deleted
    await expect(secretsService.getSecret(ctx, "lifecycle-key")).rejects.toThrow(NotFoundError);
  });
});
