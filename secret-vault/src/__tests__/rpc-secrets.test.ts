...
describe("Core secrets service via RPC context", () => {
  ...
  it("deleteSecret removes it", async () => {
    ...
    await expect(secretsService.getSecret(ctx, "rpc-test-key")).rejects.toThrow(/not found/i);
  });

  it("getSecret throws NotFoundError for missing key", async () => {
    const ctx = await buildTestCtx();
    await expect(secretsService.getSecret(ctx, "nonexistent")).rejects.toThrow(/not found/i);
  });
});
...
describe("Core versions service", () => {
  it("versioning on update", async () => {
    const ctx = await buildTestCtx();
    await secretsService.setSecret(ctx, "ver-test", {
      value: "v1",
      description: "",
      tags: "",
      expires_at: null,
  ...
  it("getVersion returns decrypted value", async () => {
    const ctx = await buildTestCtx();
    await secretsService.setSecret(ctx, "ver-get-test", {
      value: "original",
      description: "",
      tags: "",
      expires_at: null,
    ...
    const { versions } = await versionsService.listVersions(ctx, "ver-get-test");
    expect(versions.length).toBeGreaterThanOrEqual(1);
    const ver = await versionsService.getVersion(ctx, "ver-get-test", versions[0].id);
  ...
  it("restoreVersion restores a previous value", async () => {
    const ctx = await buildTestCtx();
    await secretsService.setSecret(ctx, "ver-restore", {
      value: "first",
      description: "",
      tags: "",
      expires_at: null,
    ...
    const { versions } = await versionsService.listVersions(ctx, "ver-restore");
    const result = await versionsService.restoreVersion(ctx, "ver-restore", versions[0].id);
...
describe("Core bulk service", () => {
  ...
  it("import with overwrite=false skips existing", async () => {
    const ctx = await buildTestCtx();
    await secretsService.setSecret(ctx, "bulk-skip", {
      value: "original",
      description: "",
      tags: "",
      expires_at: null,
    ...
    const result = await bulkService.importSecrets(ctx, {
      secrets: [{ key: "bulk-skip", value: "new", description: "", tags: "", expires_at: null }],
      overwrite: false,
    });
