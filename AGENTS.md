...
| CLI commands | `hfs/src/commands/` | `admin-ops.ts`, `audit.ts`, `auth.ts`, `completion.ts`, `config.ts`, `deploy.ts`, `expiring.ts`, `flags.ts`, `keygen.ts`, `profiles.ts`, `roles.ts`, `scan.ts`, `secrets-bulk.ts`, `secrets-ops.ts`, `secrets.ts`, `template.ts`, `tokens.ts`, `users.ts` |
...
| CLI helpers | `hfs/src/helpers.ts` | `die()`, `client()`, `confirm()`, `readStdin()`, `fetchAllSecrets()`, `toShellLine()`, `parseTags()` |
| CLI duration | `hfs/src/duration.ts` | Duration parsing/formatting utilities |
| CLI interpolate | `hfs/src/interpolate.ts` | String interpolation utilities |
